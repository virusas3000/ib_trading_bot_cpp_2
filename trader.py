...
Architecture:
  ...
  - 1-minute bar subscriptions for all watchlist symbols
...
def get_avg_volume_20(symbol: str) -> float:
    """Return 20-bar average volume from stored bar data (proxy)."""
    ...
    for pos in ib_positions:
        ...
        if sym in open_rows:
            ...
            row_id = db_open_position(
                ...
                avg_cost * 1.01, avg_cost * 1.02,  # placeholder targets
                "RESTORED", 0.5, 0,
            )
            state.positions[sym] = {
                "row_id": row_id, "side": side, "qty": qty_ib,
                "entry": avg_cost, "stop": stop,
                "target1": avg_cost * 1.01, "target2": avg_cost * 1.02,
...
async def place_entry(symbol: str, signal: dict, nav: float) -> None:
    ...
    entry     = signal.get("entry_price", 0.0)  # set by caller from last price
    stop_p    = signal["stop"]
    t1        = signal["target1"]
    t2        = signal["target2"]
    ...
    try:
        ...
        qty = sz.qty
        if qty == 0:
            logger.info("[SKIP_MIN_PROFIT] %s %s t1=%.2f reason=%s",
                        symbol, direction, t1, sz.reason)
            return
    except ImportError:
    ...
    await asyncio.sleep(0.5)  # brief pause for fill
    fill_price = entry  # use last price; actual fill arrives via executions
...
async def on_bar(bars, has_new_bar: bool, symbol: str) -> None:
    ...
    sig["rsi14"]       = df["close"].tolist()  # will be recomputed in place_entry
...
async def main() -> None:
    ...
    if state.nav <= 0:
        logger.error("NAV is zero — check account. Exiting.")
        return
    ...
    logger.info("Subscribing to %d symbols...", len(config.WATCHLIST))
    bar_handles = []
    for sym in config.WATCHLIST:
        contract = make_contract(sym)
        ib.qualifyContracts(contract)
        bars = ib.reqRealTimeBars(contract, 5, "TRADES", False)
        # Accumulate 5-sec bars into 1-min lists
        sym_ref = sym
        bars.updateEvent += lambda b, h, s=sym_ref: asyncio.ensure_future(on_bar(b, h, s))
        bar_handles.append(bars)

    logger.info("Watchlist subscription confirmed (%d symbols)", len(config.WATCHLIST))
